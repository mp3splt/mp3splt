                  ...
                  gpointer *data)
{
  ...
  g_snprintf(b3, 100, "-release of 27/02/06-\n%s libmp3splt 0.3.1 (%s mp3splt)", b1,b2);
...
void print_status_bar_confirmation(gint confirmation)
{
  ...
  case SPLT_ERROR_SPLITPOINTS :
    put_status_message((gchar *)_(" error: not enough splitpoints (<2) "));
    break;
  ...
  case SPLT_ERROR_SPLITPOINTS_NOT_IN_ORDER :
    put_status_message((gchar *)_(" error: splitpoints not in order "));
    break;
  case SPLT_ERROR_NEGATIVE_SPLITPOINT :
    put_status_message((gchar *)_(" error: negative splitpoint "));
    break;
  case SPLT_ERROR_INCORRECT_PATH :
    put_status_message((gchar *)_(" error: incorrect destination folder "));
    break;
  case SPLT_FREEDB_OK :
    put_status_message((gchar *)_(" freedb search processed ok "));
    break;
  case SPLT_FREEDB_FILE_OK :
    put_status_message((gchar *)_(" freedb file processed ok "));
    break;
  case SPLT_FREEDB_CDDB_OK :
    put_status_message((gchar *)_(" cddb file processed ok "));
    break;
  case SPLT_FREEDB_CUE_OK :
    put_status_message((gchar *)_(" cue file processed ok "));
    break;
  case SPLT_FREEDB_ERROR_INITIALISE_SOCKET :
    put_status_message((gchar *)_(" error: cannot initialise socket "));
    break;
  case SPLT_FREEDB_ERROR_CANNOT_GET_HOST :
    put_status_message((gchar *)_(" error: cannot get host by name "));
    break;
  case SPLT_FREEDB_ERROR_CANNOT_OPEN_SOCKET :
    put_status_message((gchar *)_(" error: cannot open socket "));
    break;
  case SPLT_FREEDB_ERROR_CANNOT_CONNECT :
    put_status_message((gchar *)_(" error: cannot connect to host "));
    break;
  case SPLT_FREEDB_ERROR_CANNOT_SEND_MESSAGE :
    put_status_message((gchar *)_(" error: cannot send message to host "));
    break;
  case SPLT_FREEDB_ERROR_INVALID_SERVER_ANSWER :
    put_status_message((gchar *)_(" error: invalid server answer "));
    break;
  case SPLT_FREEDB_ERROR_SITE_201 :
    put_status_message((gchar *)_(" error: error 201 while connecting"
                                  " to site "));
    break;
  case SPLT_FREEDB_ERROR_SITE_200 :
    put_status_message((gchar *)_(" error: error 200 while connecting"
                                  " to site "));
    break;
  case SPLT_FREEDB_ERROR_BAD_COMMUNICATION :
    put_status_message((gchar *)_(" error: bad communication with site "));
    break;
  case SPLT_FREEDB_ERROR_GETTING_INFOS :
    put_status_message((gchar *)_(" error: could not get infos from site "));
    break;
  case SPLT_FREEDB_NO_CD_FOUND :
    put_status_message((gchar *)_(" no cd found for this search "));
    break;
  case SPLT_FREEDB_MAX_CD_REACHED :
    put_status_message((gchar *)_(" maximum number of found"
                                  " CD reached "));
    break;
  case SPLT_CUE_ERROR_CANNOT_OPEN_FILE_READING:
    put_status_message((gchar *)_(" cue error: cannot read "
                                  "file"));
    break;
  case SPLT_CDDB_ERROR_CANNOT_OPEN_FILE_READING:
    put_status_message((gchar *)_(" cddb error: cannot read "
                                  "file"));
    break;
  case SPLT_INVALID_CUE_FILE:
    put_status_message((gchar *)_(" cue error: invalid "
                                  "cue file"));
    break;
  case SPLT_INVALID_CDDB_FILE:
    put_status_message((gchar *)_(" cddb error: invalid "
                                  "cddb file"));
    break;
  ...
  case SPLT_ERROR_INCOMPATIBLE_OPTIONS:
    put_status_message((gchar *)_(" error: incompatible options "));
    break;
  ...
  case SPLT_MP3_ERR_TOO_MANY_SYNC_ERR:
    ...
    break;
  case SPLT_DEWRAP_OK:
    put_status_message((gchar *)_(" wrap split ok "));
    break;
  case SPLT_DEWRAP_ERR_FILE_LENGTH:
    put_status_message((gchar *)_(" wrap error: file length "));
    break;
  case SPLT_DEWRAP_ERR_VERSION_OLD:
    put_status_message((gchar *)_(" wrap error: version too old "));
    break;
  case SPLT_DEWRAP_ERR_NO_FILE_OR_BAD_INDEX:
    put_status_message((gchar *)_(" wrap error: no file or bad "
                                  "index"));
    break;
  case SPLT_DEWRAP_ERR_FILE_DAMAGED_INCOMPLETE:
    put_status_message((gchar *)_(" wrap error: file damaged or "
                                  "incomplete"));
    break;
  case SPLT_DEWRAP_ERR_FILE_NOT_WRAPED_DAMAGED:
    put_status_message((gchar *)_(" wrap error: maybe not a wrapped"
                                  " file or wrap file damaged")); 
    break;
  case SPLT_TIME_SPLIT_OK:
    put_status_message((gchar *)_(" time split ok ")); 
    break;
  case SPLT_ERROR_INPUT_OUTPUT_SAME_FILE:
    put_status_message((gchar *)_(" error: input and output are the same file ")); 
    break;
  case SPLT_NO_SILENCE_SPLITPOINTS_FOUND:
    put_status_message((gchar *)_(" no silence splitpoints found "));
    break;
  ...
  case SPLT_ERROR_CANNOT_OPEN_DEST_FILE:
    put_status_message((gchar *)_(" error: cannot open"
                                  " destination file "));
    break;
  case SPLT_ERROR_CANT_WRITE_TO_OUTPUT_FILE:
    put_status_message((gchar *)_(" error: cannot write"
                                  " to output file "));
    break;
  case SPLT_ERROR_WHILE_READING_FILE:
    put_status_message((gchar *)_(" error: cannot read"
                                  " file "));
    break;
  case SPLT_ERROR_SEEKING_FILE:
    put_status_message((gchar *)_(" error: cannot seek"
                                  " file "));
    break;
  case SPLT_ERROR_BEGIN_OUT_OF_FILE:
    put_status_message((gchar *)_(" error: begin point out"
                                  " of file "));
    break;
  case SPLT_ERROR_INEXISTENT_FILE:
    put_status_message((gchar *)_(" error: inexistent file "));
    break;
  case SPLT_SPLIT_CANCELLED:
    put_status_message((gchar *)_(" split process cancelled "));
    break;
  case SPLT_OUTPUT_FORMAT_OK:
    remove_status_message();
    break;
  case SPLT_OUTPUT_FORMAT_ERROR:
    put_status_message((gchar *)_(" warning: output format error "));
    break;
  case SPLT_OUTPUT_FORMAT_AMBIGUOUS:
    put_status_message((gchar *)_(" warning: output format ambiguous"));
    break;
  default:
    break;
  }  
