...
/*int main()
{
  ...
  splt_pair *p = splt_pair_new((void *) &my_int, (void *) &my_long);

  int out_int = *((int *)splt_pair_first(p));
  long out_long = *((long *)splt_pair_second(p));

  fprintf(stdout,"out int = %d\n",out_int);
  fprintf(stdout,"out long = %ld\n",out_long);
  fflush(stdout);
