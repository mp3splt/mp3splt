                                             ...
                                             int *error)
{
  ...
  if ((strcmp(new_filename_path, "") != 0) &&
      (strcmp(new_filename_path, current_directory) != 0))
    {
      ...
      //-1 means error
      if((status = stat(new_filename_path, &buffer))
         == -1)
        {
...
void splt_check_set_correct_options(splt_state *state)
{
  ...
  if (( split_mode == SPLT_OPTION_SILENCE_MODE)
      || splt_t_get_int_option(state,SPLT_OPT_AUTO_ADJUST))
    {
      ...
      if  ((splt_t_get_float_option(state,SPLT_OPT_PARAM_THRESHOLD)
            <-96.f) || 
      ...
      if  ((splt_t_get_float_option(state,SPLT_OPT_PARAM_OFFSET)
            <-2.f) || 
      ...
      //for the adjust mode
      if (splt_t_get_int_option(state,SPLT_OPT_PARAM_GAP)
          < 0)
        {
          splt_t_set_int_option(state,SPLT_OPT_PARAM_GAP,
                                SPLT_DEFAULT_PARAM_GAP);
        }
      //for the silence mode
      if (splt_t_get_float_option(state,SPLT_OPT_PARAM_MIN_LENGTH)
          <0.f)
        {
  ...
  if ((splt_t_get_int_option(state,SPLT_OPT_INPUT_NOT_SEEKABLE)) &&
    ...
    {
      splt_t_set_int_option(state,SPLT_OPT_INPUT_NOT_SEEKABLE,
                            SPLT_FALSE);
    }
}
